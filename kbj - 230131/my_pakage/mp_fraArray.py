...
class FRAArray:
    def __init__(
        ...
        row_array_from: int = 0,
        row_array_to: int = 301,
        column_array_from: int = 1,
        column_array_to: int = 2,
    ):
        self._root_name = root_name
        self._array_file_extension = array_file_extension
        self._folder_array_setting = folder_array_setting
        self._file_name = file_name
        self._array_from = row_array_from
        self._array_to = row_array_to
        self._column_array_from = column_array_from
        self._column_array_to = column_array_to
        self.array_result = []
    ...
    def fra_file_array(self, folder_root_name):
    ...
    def fra_file_array(self, folder_root_name):
        ...
        self._column_array_from -= 1

        if self._column_array_from < 0:
            self._column_array_from = 0
        ...
        for file in all_file_list:
            ...
            if self._array_file_extension == "txt":
                df = pd.read_table(file, skiprows=self._array_from, nrows=self._array_to)
                temp = list(set(df.columns) - set(df.columns[self._column_array_from : self._column_array_to]))
    ...
    def fra_array_start(self):
        ...
        if self._folder_array_setting == 1:
            ...
            with Timer():
                result = self.fra_file_array(self._root_name)
            ...
            with Timer():
                ...
                thread_list = [threading.Thread(target=self.fra_file_array, args=(folder,)) for folder in folder_list]
